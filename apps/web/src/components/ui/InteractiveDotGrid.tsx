...
export const InteractiveDotGrid: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      dots.forEach((dot) => {
        const waveX = Math.sin(time + dot.originY * 0.035 + dot.originX * 0.02) * 4;
        const waveY = Math.cos(time + dot.originX * 0.035 + dot.originY * 0.02) * 4;
        ...
        // Repel / displacement on mouse or touch hover
        if (dist < mouse.radius) {
          const force = (mouse.radius - dist) / mouse.radius;
          const angle = Math.atan2(dy, dx);
          const targetX = targetOriginX - Math.cos(angle) * force * 24;
          const targetY = targetOriginY - Math.sin(angle) * force * 24;
          dot.vx += (targetX - dot.x) * 0.15;
          dot.vy += (targetY - dot.y) * 0.15;
        } else {
          dot.vx += (targetOriginX - dot.x) * 0.08;
          dot.vy += (targetOriginY - dot.y) * 0.08;
        }
        ...
        if (dist < mouse.radius) {
          ...
          ctx.fillStyle = `rgba(200, 205, 215, ${Math.max(0.18, Math.min(0.55, alpha))})`;
        }
        ctx.fill();
